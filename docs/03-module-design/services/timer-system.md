...
**避免循环依赖**: Timer System → Event Loop → Hook/Skill Engine → Timer System

Timer System 与 Hook/Skill Engine 之间存在潜在循环依赖风险：
...
**约束**：
1. **Hook/Skill 实现不得调用 Timer System API** - Hook 和 Skill 的实现代码不应该直接调用 Timer System 的任何接口，否则会导致循环依赖
2. **Event Loop 仅负责事件分发** - Event Loop 不会将控制权返回给 Timer System，定时器的后续执行由调用方发起
3. **Timer System 仅发送事件** - Timer System 通过 Event Loop 发送事件后不等待返回值

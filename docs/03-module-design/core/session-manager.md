...
| 依赖模块 | 依赖类型 | 说明 |
|---------|---------|------|
| Logging System | 依赖 | 记录会话日志。见 [Logging System 接口](./logging-system.md) |
| Storage Service | 依赖 | 会话数据持久化。见 [Storage Service 接口](./storage-service.md) |
| Context Compressor | 依赖 | 上下文压缩。见 [Context Compressor 接口](./context-compressor.md) |
| Security Manager | 依赖 | 路径权限检查。见 [Security Manager 接口](../security/security-manager.md) |
...
| 1.1.0 | 2026-04-01 | 添加与 Context Compressor 的职责划分说明 |
| 2.0.0 | 2026-04-08 | 添加进程级隔离设计 |

### 进程管理接口

Session Manager 在守护进程中运行，负责管理会话进程的完整生命周期。

```yaml
session_manager:
  # 进程管理
  spawn_session_process:
    description: 创建并启动会话进程
    inputs:
      session_id: string
      workspace: string
      config: SessionProcessConfig
    outputs:
      process_handle: SessionProcessHandle
  ...
  list_processes:
    description: 列出所有会话进程
    outputs:
      processes: array<SessionProcessInfo>
```

### 进程数据结构

```yaml
SessionProcessConfig:
  session_id: string
  workspace: string
  binary_path: string          # 会话进程可执行文件路径
  args: array<string>           # 启动参数
  env_vars: map                 # 环境变量
  resource_limits:
    max_memory_mb: integer       # 最大内存 (MB)
    max_cpu_percent: integer     # 最大 CPU (%)

SessionProcessHandle:
  session_id: string
  pid: integer
  ipc_channel: IpcChannel       # 与会话进程的 IPC 通道
  status: ProcessStatus         # running/stopped/crashed
  started_at: datetime

ProcessStatus:
  type: enum
  values: [starting, running, stopping, stopped, crashed, recovering]
```
...
```
创建:
  Session Manager → spawn_session_process()
    │
    ├── fork/exec 会话进程 (knight-agent session --id <id>)
    ├── 建立 IPC 通道 (Unix Socket)
    ├── 等待会话进程 Bootstrap 完成
    └── 注册到 Process Monitor

运行:
  Process Monitor → 定期健康检查
    │
    ├── 心跳: 会话进程定期发送 heartbeat
    ├── 资源: 监控内存/CPU 使用
    └── 状态: 接收状态更新事件

崩溃恢复:
  Process Monitor → 检测到心跳超时
    │
    ├── 标记会话为 crashed
    ├── 从最后一个 checkpoint 恢复
    └── restart_session_process()
...
销毁:
  Session Manager → terminate_session_process()
    │
    ├── 发送 shutdown IPC 消息
    ├── 等待优雅关闭 (timeout)
    ├── 超时则 force kill
...
- 每个会话运行在独立的操作系统进程中
- 进程间无法直接访问内存
- Workspace 路径限制在进程级执行
- 一个会话崩溃不影响其他会话
- 资源限制通过 OS 级别控制 (cgroups / job objects)

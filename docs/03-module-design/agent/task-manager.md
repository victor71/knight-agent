...
| 依赖模块 | 依赖类型 | 说明 |
|---------|---------|------|
| Orchestrator | 协作 | 调用 [Agent 分配接口](../core/orchestrator.md#agent-分配-供-task-manager-调用) |
| Agent Runtime | 依赖 | 执行任务。见 [Agent Runtime 接口](./agent-runtime.md) |
| Storage Service | 依赖 | 持久化任务状态和检查点。见 [Storage Service 接口](./storage-service.md) |
| Skill Engine | 依赖 | 技能任务执行。见 [Skill Engine 接口](./skill-engine.md) |
| LLM Provider | 协作 | 用于 Command 触发的工作流解析 |
| Command | 被依赖 | Command 模块调用 [create_workflow_from_parsed](./task-manager.md) |

...
│                                                                    │
│  ┌──────────────────┐                                             │
│  │  knight (CLI)    │  ← 用户在 Shell 中执行                      │
│  │  系统管理命令     │    knight daemon start/stop/status          │
│  │  快速询问         │    knight ask <agent> "<message>"          │
│  └────────┬─────────┘                                             │
│           │                                                        │
...
| 进程类型 | 职责 | 启动方式 |
|---------|------|---------|
| **knight (CLI)** | 系统管理命令、快速询问，用完即退出 | 用户在 Shell 中执行 |
| **knight-agent (Daemon)** | 守护进程，包含 TUI、Session Manager、Router 等 | `knight daemon start` 或 `knight` |
| **Session Process** | 会话进程，包含 Agent Runtime、Orchestrator、Tools 等 | 守护进程按需 spawn |

**IPC 边界**:

| IPC 通道 | 通信双方 | 协议 | 用途 |
|---------|---------|------|------|
| IPC #1 | knight CLI ↔ knight-agent Daemon | Unix Socket / TCP | CLI 命令转发、状态查询 |
...
└─────────┬─────────┴────────┬─────────┴──────────┬──────────────┘
          │                  │                     │
          │  IPC Contract    │  mpsc channels      │  gRPC/WebSocket
          │                  │                     │
          ▼                  ▼                     ▼
┌─────────────────────────────────────────────────────────────────┐
│                     核心引擎层 (Core Engine)                     │
...
| 模块 | 守护进程 (Daemon) | 会话进程 (Session) | CLI 进程 |
|------|:-:|:-:|:-:|
| TUI | ✅ | - | - |
| Session Manager | ✅ | - | - |
| Process Monitor | ✅ | - | - |

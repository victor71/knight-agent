...
Knight Agent CLI 支持两种交互模式：**TUI (终端用户界面)** 和 **REPL (命令行界面)**。
...
```bash
knight                          # 启动 REPL（自动启动守护进程）
knight daemon start/stop/status/restart   # 守护进程管理
knight health                            # 健康检查
knight version                            # 版本信息
```
...
**新增 P1 功能**: 类似 Claude Code 的交互式 TUI 界面

| 需求项 | 描述 | 优先级 |
|--------|------|--------|
| **状态栏显示** | 完整系统信息显示 | P1 |
| **Session 管理** | Header 显示当前 Session，支持快捷切换 | P1 |
| **Task 列表** | 弹窗显示所有任务及运行时长 | P1 |
| **当前任务计时** | 状态栏显示当前任务实时计时器 | P1 |
| **Token 使用统计** | 状态栏显示当前会话 Token 使用量 | P1 |
| **上下文压缩状态** | 显示当前上下文大小及压缩阈值 | P1 |
| **富文本输出** | 支持 Markdown/代码高亮/流式输出 | P1 |
| **可编辑输入** | Readline 风格的命令输入 | P1 |

**状态栏显示内容**:
```
┌────────────────────────────────────────────────────────┐
│ 📦 D:/workspace/knight-agent     [main]     14:32:05  │
│ Session: abc123-def4  [+ New]  [- Switch]             │
│ Agents: 5 | Tasks: 3 | Memory: 256MB                   │
├────────────────────────────────────────────────────────┤
│ knight> help                              [INSERT]     │
├────────────────────────────────────────────────────────┤
│ 🟢 Running | 🔄 code-reviewer 00:02:34 | Tasks: 2     │
│ Tokens: 12,345/200K (6%) | Context: 12MB/25MB (48%)   │
└────────────────────────────────────────────────────────┘
```

**Session 切换**:
- Header 显示当前 Session ID
- `Alt+N` - 创建新 Session
- `Alt+S` - 打开 Session 切换弹窗
- Session 弹窗显示所有会话列表

**Task 列表**:
- `Alt+T` - 打开 Task 列表弹窗
- 显示所有任务状态 (🔄 运行中 / ⏳ 等待 / ✅ 完成 / ❌ 失败)
- 显示任务运行时长 (实时更新)
...
**Token 使用统计**:
- 格式: `Tokens: 12,345/200K (6%)`
- 颜色指示: 绿色 < 50%, 黄色 < 80%, 红色 >= 80%

**上下文压缩状态**:
- 格式: `Context: 12MB/25MB (48%)` 或 `Context: 19MB/25MB (75% - ⚠ 25% until compression)`
- 颜色指示: 绿色 < 70%, 黄色 < 90%, 红色 >= 90%
...
| 需求项 | 描述 | 优先级 |
|--------|------|--------|
| 自然语言定时 | 用户用自然语言描述定时任务 | P1 |
| 定时任务创建 | 支持一次性、周期性、递归定时任务 | P0 |
| 多任务管理 | 同时管理多个定时任务 | P1 |
| 任务持久化 | 定时任务保存到磁盘，重启后恢复 | P1 |
| 定时取消/修改 | 支持取消和修改已创建的定时任务 | P1 |
| 定时执行日志 | 记录定时任务的执行结果 | P1 |
...
```yaml
# config/schedules/news-digest.yaml
schedule:
  id: task-001
  name: AI新闻简报
  description: 每天早上8点收集并发送AI新闻简报
  ...
  agent: news-digester
  prompt: |
    请帮我收集昨天的AI新闻，整理成简报格式。
    包含: 技术突破、产品更新、行业动态。
    格式: Markdown，控制在500字以内。
  ...
  notify:
    - type: email
      to: user@example.com
    - type: webhook
      url: https://team.example.com/notify
...
| 类型 | 描述 | 示例 |
|------|------|------|
| **一次性** | 指定时间执行一次 | "2小时后提醒我" |
| **周期性** | 按 cron 表达式重复 | "每天早上8点" |
| **递归** | 间隔重复执行 | "每6小时检查一次" |
...
| 阶段 | 目标 | 交付物 | 验收标准 |
|------|------|--------|----------|
| Phase 1 | 核心 Agent 框架 | Agent 引擎、基础工具、会话管理 | Agent 可以响应指令，多会话并行 |
| Phase 2 | Skill 系统 + 会话增强 | Skill 引擎、上下文压缩、存储服务 | Skill 可以被触发，会话可持久化 |
| Phase 3 | 协作能力 | 多 Agent、消息传递、日志权限 | 两个 Agent 协作，有日志和权限控制 |
| Phase 4 | 自动化 | 事件循环、定时任务、守护进程 | 定时任务自动执行 |
| Phase 5 | 生态 | MCP 集成、插件系统、模板库 | MCP 工具可用，支持插件扩展 |
